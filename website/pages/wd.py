...
def generate_word_cloud(channel):
    ...
    if not titles:  # No new titles found, use old titles as fallback
        # Retrieve old titles for the channel
        old_videos = Video.objects.filter(channel=channel)
        titles = [video.title for video in old_videos]

    if not titles:  # No titles available for the channel
        return  # Skip generating the word cloud for this channel

    # Join the titles to create the text data
    text = ' '.join(titles)

    # Read the stop words from a file
    with open('/home/rohit/news/website/static/function/stop.txt', 'r', encoding='utf8') as stop_file:
    ...
    # Load the mask image
    python_mask = np.array(PIL.Image.open(f"/home/rohit/news/website/static/function/logos/{channel}.jpg"))
    colormap = ImageColorGenerator(python_mask)

    # Generate the word cloud
    wordcloud = WordCloud(font_path='/home/rohit/news/website/static/function/Arya-Regular.ttf',
                          mask=python_mask,
                          min_word_length=200,
    ...
    with open(f"/home/rohit/news/website/static/function/clouds/{channel}.svg", "w+", encoding='UTF-8') as f:
        f.write(svg_image)
    ...
    png_image = plt.savefig(f"/home/rohit/news/website/static/function/clouds/png/{channel}.png")
